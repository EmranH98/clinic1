...
async function pgDailyEntry() {
  ...
  document.getElementById('pageContent').innerHTML = `
    ...
    <div class="flex-center gap8 mb16" style="flex-wrap:wrap">
      ...
      <!-- Service type filter -->
      <select id="opsTypeFilter" style="padding:8px 12px;border:1.5px solid var(--border);border-radius:8px;font-family:inherit;font-size:13px" onchange="applyOpsClientFilter()">
      ...
      <select id="opsStaffFilter" style="padding:8px 12px;border:1.5px solid var(--border);border-radius:8px;font-family:inherit;font-size:13px" onchange="loadOps()">
        ...
        <optgroup label="Doctors">
          ${getDoctors().map(d=>`<option value="${d.id}" data-role="doctor">${d.name}</option>`).join('')}
        </optgroup>
        <optgroup label="Specialists">
          ${getSpecialists().map(s=>`<option value="${s.id}" data-role="specialist">${s.name}</option>`).join('')}
        </optgroup>
      </select>` : ''}
...
// Client-side filter by service type and gender (derived from procedure fields)
function applyOpsClientFilter() {
  const typeFilter   = document.getElementById('opsTypeFilter')?.value   || '';
...
function filterOpsTable(q) {
  ...
  if (typeFilter === 'specialist') filtered = filtered.filter(o => o.specialist_id && !o.doctor_id);
  else if (typeFilter === 'doctor') filtered = filtered.filter(o => o.doctor_id);
  if (genderFilter) {
    filtered = filtered.filter(o => {
