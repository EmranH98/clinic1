...
function buildOpForm(op) {
  ...
  // Detect initial specialist type (for editing an existing op)
  ...
  return `
    ...
    <div id="f-spec-row" style="display:${isSpec?'block':'none'}">
      <div class="field"><label>Specialist (الاخصائية)</label>
        <select id="f-spec" onchange="onSpecSelectChange(this.value)">
          ...
          ${specialists.map(s=>`<option value="${s.id}" ${op?.specialist_id===s.id?'selected':''}>${s.name}${s.specialist_type&&s.specialist_type!=='standard'?' ('+( s.specialist_type==='nutritionist'?'🥗 Nutritionist':'♂ Men\'s Laser')+')':''}</option>`).join('')}
        </select>
      </div>
    </div>
    ...
    <!-- Commission row (admin/manager only, specialist ops only) -->
    ${showCommission ? `
    <div id="f-commission-row" class="alert" style="background:#f0fdf4;border:1.5px solid #86efac;border-radius:8px;padding:10px 14px;margin-bottom:12px">
      <!-- Fixed commission (standard specialists) -->
      ...
      <!-- % commission (nutritionist / men_laser) -->
    ...
    <!-- Manual material cost (optional; auto-filled when items are selected above) -->
...
function onProcChange(procId) {
  ...
  // Re-apply specialist type display in case a specialist was already selected
  const specId = document.getElementById('f-spec')?.value;
  if (specId) onSpecSelectChange(specId);
}
...
async function saveOpFromModal(id) {
  ...
  try {
    ...
    // manualCost is used only when no inventory items are selected.
