...
let _opsTypeTab  = 'all';   // 'all' | 'women' | 'men' | 'nutritionist' | 'doctor'
let _opItems     = [];      // items-used rows in the op modal
let _opInventory = [];      // inventory items loaded for the modal

// ─── Shared helpers ───────────────────────────────

/** Returns comma-separated procedure names for an operation row */
function getOpProcNames(op) {
  if (op.op_procedures?.length > 0)
    return op.op_procedures.map(p => p.procedure?.name || '?').join(', ');
  return op.procedure?.name || '';
}

/**
 * Returns the doctor profit % for an operation.
 * Checks op_procedures first, falls back to legacy single procedure, then 50%.
 */
function getOpDocPct(op) {
  if (op.op_procedures?.length > 0) {
    const pct = op.op_procedures[0]?.procedure?.doctor_profit_pct;
    if (pct != null) return pct;
  }
  return op.procedure?.doctor_profit_pct ?? 50;
}

/**
 * Calculates the specialist commission for an operation.
 * - % types (nutritionist / men_laser): % of session revenue
 * - Standard: sum of procedure commissions × quantity
 */
function calcOpSpecCommission(op) {
  const spec     = op.specialist || {};
  const specType = spec.specialist_type || 'standard';
  const isPct    = ['nutritionist','men_laser'].includes(specType);
  if (isPct) {
    const total = (op.payment_cash||0)+(op.payment_visa||0)+(op.payment_cliq||0)+(op.payment_shot||0)-(op.discount||0);
    return total * ((spec.commission_rate||0) / 100);
  }
  // Standard: sum op_procedures, or fall back to legacy single-procedure
  if (op.op_procedures?.length > 0) {
    return op.op_procedures.reduce((s, p) => {
      const base = p.specialist_commission_override ?? (p.procedure?.specialist_commission ?? 0);
      return s + base * (p.quantity || 1);
    }, 0);
  }
  return op.specialist_commission_override ?? (op.procedure?.specialist_commission ?? 0);
}
...
async function pgDailyEntry() {
  ...
  document.getElementById('pageContent').innerHTML = `
    ...
    <div style="display:flex;gap:8px;margin-bottom:14px;flex-wrap:wrap">
      ...
      <button id="opsTab-women"        onclick="setOpsTab('women')"        style="${tbStyle(_opsTypeTab==='women')}">♀ Women</button>
      <button id="opsTab-men"          onclick="setOpsTab('men')"          style="${tbStyle(_opsTypeTab==='men')}">♂ Men</button>
      <button id="opsTab-nutritionist" onclick="setOpsTab('nutritionist')" style="${tbStyle(_opsTypeTab==='nutritionist')}">🥗 Nutritionist</button>
...
function setOpsTab(tab) {
  _opsTypeTab = tab;
  ['all','women','men','nutritionist','doctor'].forEach(t => {
...
// Client-side filter by section tab (All / Women / Men / Nutritionist / Doctor)
function applyOpsClientFilter() {
  applyOpsClientFilterOn(_currentOps);
}
        ...
        <tbody>
          ${ops.map(o => {
            ...
            return `<tr>
              ...
              <td><span class="fs12">${esc(o.session_name||getOpProcNames(o)||'-')}</span></td>
...
function filterOpsTable(q) {
  ...
  // Re-apply the active tab filter on top of the text search
  applyOpsClientFilterOn(filtered);
}

// Applies the active type-tab filter to a pre-filtered list and renders
function applyOpsClientFilterOn(list) {
  const nutritionistIds = new Set(
    (_profiles || []).filter(p => p.specialist_type === 'nutritionist').map(p => p.id)
  );
  const menLaserIds = new Set(
    (_profiles || []).filter(p => p.specialist_type === 'men_laser').map(p => p.id)
  );
  const opGender = o => {
    if (o.op_procedures?.length > 0) return o.op_procedures[0]?.procedure?.gender || '';
    return o.procedure?.gender || '';
  };

  let filtered = list;
  if (_opsTypeTab === 'women') {
    filtered = filtered.filter(o =>
      o.specialist_id && !o.doctor_id &&
      !nutritionistIds.has(o.specialist_id) &&
      !menLaserIds.has(o.specialist_id) &&
      opGender(o) !== 'male'
    );
  } else if (_opsTypeTab === 'men') {
    filtered = filtered.filter(o =>
      o.specialist_id && !o.doctor_id &&
      !nutritionistIds.has(o.specialist_id) &&
      (menLaserIds.has(o.specialist_id) || opGender(o) === 'male')
    );
  } else if (_opsTypeTab === 'nutritionist') {
    filtered = filtered.filter(o => o.specialist_id && nutritionistIds.has(o.specialist_id));
...
// Tracks category and procedure list for the modal
let _opCategory   = 'women';  // 'women' | 'men' | 'both' | 'nutritionist' | 'doctor'
let _opProcedures = [];       // [{procedureId, quantity, commOverride, procedure}]
...
async function openEditOpModal(id) {
  ...
  // Determine category from operation
  if (op.doctor_id && !op.specialist_id) {
    _opCategory = 'doctor';
  } else {
    const spec = (_profiles||[]).find(p => p.id === op.specialist_id);
    if (spec?.specialist_type === 'nutritionist')  _opCategory = 'nutritionist';
    else if (spec?.specialist_type === 'men_laser') _opCategory = 'men';
    else {
      // Standard specialist — infer from existing procedure gender
      const proc = _procedures.find(p => p.id === op.procedure_id);
      _opCategory = proc?.gender === 'male' ? 'men' : 'women';
    }
  }

  // Load existing procedure list (op_procedures if present, else legacy single procedure)
  _opProcedures = (op.op_procedures?.length > 0)
    ? op.op_procedures.map(p => ({
        procedureId:  p.procedure_id,
        quantity:     p.quantity || 1,
        commOverride: p.specialist_commission_override || null,
        procedure:    p.procedure || null
      }))
    : op.procedure_id
      ? [{ procedureId: op.procedure_id, quantity: 1, commOverride: null,
           procedure: _procedures.find(p => p.id === op.procedure_id) || null }]
      : [];
...
function buildOpForm(op) {
  ...
  const isDoc    = _opCategory === 'doctor';
  const isAdmin_ = isAdmin() || isManager();

  // Build initial specialist dropdown options based on category
  const catSpecialists = _buildCatSpecialists();

  // Detect initial specialist for commission display
  const initSpec     = op?.specialist_id ? (_profiles||[]).find(p => p.id === op.specialist_id) : null;
  const initIsPct    = ['nutritionist','men_laser'].includes(initSpec?.specialist_type);
  const initSpecType = initSpec?.specialist_type || 'standard';

  const catStyle = (cat) =>
    `padding:9px 14px;border-radius:8px;font-size:13px;border:2px solid;cursor:pointer;transition:.15s;` +
    (cat === _opCategory
  ...
  return `
    ...
    <div style="display:flex;gap:8px;margin-bottom:16px;flex-wrap:wrap">
      <button id="opCat-women"        type="button" onclick="setOpCategory('women')"        style="${catStyle('women')}">♀ Women</button>
      <button id="opCat-men"          type="button" onclick="setOpCategory('men')"          style="${catStyle('men')}">♂ Men</button>
      <button id="opCat-both"         type="button" onclick="setOpCategory('both')"         style="${catStyle('both')}">⚥ Both</button>
      <button id="opCat-nutritionist" type="button" onclick="setOpCategory('nutritionist')" style="${catStyle('nutritionist')}">🥗 Nutritionist</button>
      <button id="opCat-doctor"       type="button" onclick="setOpCategory('doctor')"       style="${catStyle('doctor')}">👨‍⚕️ Doctor</button>
    </div>
    ...
    <!-- Specialist row (non-doctor categories) -->
    <div id="f-spec-row" style="display:${isDoc?'none':'block'}">
      <div class="field"><label>Specialist (الاخصائية)</label>
        <select id="f-spec" onchange="onSpecSelectChange(this.value)">
          <option value="">— Select specialist —</option>
          ${catSpecialists.map(s=>`<option value="${s.id}" ${op?.specialist_id===s.id?'selected':''}>${s.name}</option>`).join('')}
        </select>
      </div>
    </div>

    <!-- Doctor row -->
    <div id="f-doc-row" style="display:${isDoc?'block':'none'}">
    ...
    <!-- Commission info (admin/manager, non-doctor) -->
    ${isAdmin_ && !isDoc ? `
    <div id="f-commission-row" style="background:#f0fdf4;border:1.5px solid #86efac;border-radius:8px;padding:10px 14px;margin-bottom:12px">
      <div id="f-comm-fixed-info" style="display:${initIsPct?'none':'flex'};align-items:center;gap:12px;flex-wrap:wrap">
        <span style="font-size:13px;color:var(--ink2)">🔒 <strong>Specialist Commission</strong> <span style="font-weight:400;font-size:12px;color:var(--ink3)">(admin/manager)</span></span>
        <span style="font-size:12px;color:var(--ink3)">Auto-calculated from procedure list below ↓</span>
        <span style="font-size:12px;color:var(--teal);font-weight:600" id="f-proc-comm-info"></span>
      </div>
      <div id="f-comm-pct-info" style="display:${initIsPct?'flex':'none'};align-items:center;gap:12px;flex-wrap:wrap">
        <span id="f-comm-pct-label" style="font-size:13px;color:var(--ink2)">📊 <strong>${initSpecType==='nutritionist'?'🥗 Nutritionist Commission':'♂ Men\'s Laser Commission'}</strong></span>
        <span id="f-comm-pct-badge" class="badge bg-amber" style="font-size:13px">${initIsPct?(initSpec?.commission_rate||0)+'% of session total':'0% of session total'}</span>
        <span style="font-size:12px;color:var(--ink3)">— auto-calculated from session revenue</span>
      </div>
    </div>` : ''}
    ...
    <div style="border:1.5px solid var(--border);border-radius:10px;padding:14px;margin-bottom:14px">
      <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:10px">
        <strong style="font-size:13px;color:var(--ink2)">📋 Procedures / Services</strong>
        <button type="button" onclick="addOpProc()" style="padding:4px 12px;border-radius:6px;border:1.5px solid var(--teal);background:var(--teal-light);color:var(--teal);font-size:12px;font-weight:600;cursor:pointer">+ Add Procedure</button>
      </div>
      <div id="f-op-procs">
        ${_opProcedures.length === 0
          ? `<div id="f-op-procs-empty" style="color:var(--ink3);font-size:12px;padding:6px 0">Click "+ Add Procedure" to add a service.</div>`
          : _opProcedures.map((p, i) => buildOpProcRow(i, p)).join('')}
      </div>
      <div style="text-align:right;font-size:13px;margin-top:8px;color:var(--ink3)">
        Commission Total: <strong id="f-proc-comm-total" style="color:var(--teal)">0.00 ${CURRENCY}</strong>
      </div>
    </div>
    ...
    <!-- Manual material cost -->
    <div style="display:flex;align-items:center;gap:10px;margin-bottom:14px;padding:10px 14px;background:var(--bg2);border-radius:8px;border:1.5px solid var(--border)">
      <span style="font-size:13px;font-weight:600;color:var(--ink2);white-space:nowrap">💰 Material Cost</span>
      <input type="number" id="f-manual-cost" value="${op?.material_cost||0}" min="0" step="0.01" placeholder="0.00"
        style="padding:7px 10px;border:1.5px solid var(--border);border-radius:7px;font-size:14px;font-weight:600;width:110px;text-align:right">
      <span id="f-manual-cost-hint" style="font-size:12px;color:var(--ink3)">${CURRENCY} — optional, auto-filled when using items above</span>
    </div>
...
// Returns the specialist list filtered by current _opCategory
function _buildCatSpecialists() {
  const all = getSpecialists();
  if (_opCategory === 'men')          return all.filter(s => s.specialist_type === 'men_laser');
  if (_opCategory === 'nutritionist') return all.filter(s => s.specialist_type === 'nutritionist');
  // women / both → standard specialists
  return all.filter(s => !s.specialist_type || s.specialist_type === 'standard');
}

// ─── Procedure list helpers ───────────────────────
function buildOpProcRow(idx, proc = {}) {
  const filtered = _filterProcsByCategory();
  const opts = filtered.map(p =>
    `<option value="${esc(p.id)}" data-comm="${p.specialist_commission||0}" ${proc.procedureId===p.id?'selected':''}>${esc(p.name)}${p.name_ar?' / '+esc(p.name_ar):''}</option>`
  ).join('');
  return `
    <div id="op-proc-row-${idx}" style="display:grid;grid-template-columns:1fr 70px auto;gap:8px;align-items:center;margin-bottom:8px">
      <select onchange="onOpProcSelect(${idx},this.value,this)" style="padding:6px 8px;border:1.5px solid var(--border);border-radius:7px;font-size:13px">
        <option value="">— Select service —</option>
        ${opts}
      </select>
      <input type="number" id="op-proc-qty-${idx}" value="${proc.quantity||1}" min="1" step="1" placeholder="Qty"
        style="padding:6px 8px;border:1.5px solid var(--border);border-radius:7px;font-size:13px;text-align:right"
        oninput="calcProcCommTotal()">
      <button type="button" onclick="removeOpProc(${idx})" style="padding:4px 10px;border-radius:6px;border:1.5px solid var(--border);background:#fff;color:var(--red);cursor:pointer;font-size:13px">🗑</button>
    </div>
  `;
}

function addOpProc() {
  const emptyEl = document.getElementById('f-op-procs-empty');
  if (emptyEl) emptyEl.remove();
  const idx = _opProcedures.length;
  _opProcedures.push({ procedureId: '', quantity: 1, commOverride: null, procedure: null });
  const container = document.getElementById('f-op-procs');
  if (container) container.insertAdjacentHTML('beforeend', buildOpProcRow(idx, _opProcedures[idx]));
}

function removeOpProc(idx) {
  _opProcedures.splice(idx, 1);
  const container = document.getElementById('f-op-procs');
  if (!container) return;
  if (_opProcedures.length === 0) {
    container.innerHTML = `<div id="f-op-procs-empty" style="color:var(--ink3);font-size:12px;padding:6px 0">Click "+ Add Procedure" to add a service.</div>`;
  } else {
    container.innerHTML = _opProcedures.map((p, i) => buildOpProcRow(i, p)).join('');
  }
  calcProcCommTotal();
}

function onOpProcSelect(idx, procId) {
  const proc = _procedures.find(p => p.id === procId);
  if (_opProcedures[idx]) {
    _opProcedures[idx].procedureId = procId;
    _opProcedures[idx].procedure   = proc || null;
  }
  calcProcCommTotal();
}

function calcProcCommTotal() {
  let total = 0;
  _opProcedures.forEach((_, idx) => {
    const sel  = document.querySelector(`#op-proc-row-${idx} select`);
    const qty  = parseInt(document.getElementById(`op-proc-qty-${idx}`)?.value || 1);
    const proc = sel ? _procedures.find(p => p.id === sel.value) : null;
    total += (proc?.specialist_commission || 0) * qty;
  });
  const el = document.getElementById('f-proc-comm-total');
  if (el) el.textContent = `${total.toFixed(2)} ${CURRENCY}`;
  const info = document.getElementById('f-proc-comm-info');
  if (info && total > 0) info.textContent = `Sum: ${total.toFixed(2)} ${CURRENCY}`;
}

function collectOpProcedures() {
  return _opProcedures.map((_, idx) => {
    const sel = document.querySelector(`#op-proc-row-${idx} select`);
    const qty = parseInt(document.getElementById(`op-proc-qty-${idx}`)?.value || 1);
    return { procedureId: sel?.value || '', quantity: qty };
...
function setOpCategory(cat) {
  _opCategory = cat;
  const isDoc = cat === 'doctor';

  // Update button highlight
  ['women','men','both','nutritionist','doctor'].forEach(c => {
    const btn = document.getElementById(`opCat-${c}`);
    if (!btn) return;
    const active = c === cat;
    btn.style.background  = active ? 'var(--teal)' : '#fff';
    btn.style.color       = active ? '#fff'        : 'var(--ink2)';
  ...
  // Show/hide specialist vs doctor row
  ...
  if (specRow) specRow.style.display = isDoc ? 'none' : 'block';
  if (docRow)  docRow.style.display  = isDoc ? 'block' : 'none';
  if (commRow) commRow.style.display = (!isDoc && (isAdmin()||isManager())) ? '' : 'none';

  // Rebuild specialist options and procedures
...
function updateSpecialistOptions() {
  ...
  if (!sel) return;
  const specialists = _buildCatSpecialists();
...
// Returns procedures filtered by current _opCategory
function _filterProcsByCategory() {
  return (_procedures || []).filter(p => {
    const st = p.service_type || 'specialist';
    const gn = p.gender || '';
    if (_opCategory === 'doctor')       return ['doctor','both'].includes(st);
    if (_opCategory === 'nutritionist') return st === 'nutritionist';
    if (_opCategory === 'women')        return ['specialist','both'].includes(st) && ['female','unisex',''].includes(gn);
    if (_opCategory === 'men')          return ['specialist','both'].includes(st) && ['male','unisex'].includes(gn) || (st === 'specialist' && !gn);
    if (_opCategory === 'both')         return ['specialist','both'].includes(st) && (gn === 'unisex' || !gn);
    return true;
  });
}

// Re-render procedure option lists inside existing proc rows (after category change)
function refreshOpProcRows() {
  const container = document.getElementById('f-op-procs');
  if (!container) return;
  if (_opProcedures.length === 0) {
    container.innerHTML = `<div id="f-op-procs-empty" style="color:var(--ink3);font-size:12px;padding:6px 0">Click "+ Add Procedure" to add a service.</div>`;
...
// Legacy alias kept for onProcChange compatibility
function updateOpProcDropdown() { refreshOpProcRows(); }

// onProcChange is now handled per-row by onOpProcSelect()

// ─── Dynamic commission display by specialist type ────
function onSpecSelectChange(specId) {
  const spec     = specId ? (_profiles||[]).find(p => p.id === specId) : null;
  const specType = spec?.specialist_type || 'standard';
  const isPct    = specType === 'nutritionist' || specType === 'men_laser';

  const fixedDiv = document.getElementById('f-comm-fixed-info');
  const pctDiv   = document.getElementById('f-comm-pct-info');
  if (!fixedDiv && !pctDiv) return; // commission row not visible (non-admin)

  if (fixedDiv) fixedDiv.style.display = isPct ? 'none' : 'flex';
  if (pctDiv)   pctDiv.style.display   = isPct ? 'flex' : 'none';

  if (isPct && spec) {
    const lbl = document.getElementById('f-comm-pct-label');
    const bdg = document.getElementById('f-comm-pct-badge');
    if (lbl) lbl.innerHTML = `📊 <strong>${specType === 'nutritionist' ? '🥗 Nutritionist Commission' : '♂ Men\'s Laser Commission'}</strong>`;
    if (bdg) bdg.textContent = `${spec.commission_rate || 0}% of session total`;
  }
}
...
function calcItemsTotal() {
  ...
  // Keep the manual cost field in sync with the items total
  const manualEl = document.getElementById('f-manual-cost');
  const hintEl   = document.getElementById('f-manual-cost-hint');
  if (manualEl) {
    if (_opItems.length > 0) {
      // Items selected → auto-fill and lock the field
      manualEl.value    = total.toFixed(2);
      manualEl.readOnly = true;
      manualEl.style.background = 'var(--bg2)';
      manualEl.style.color      = 'var(--ink3)';
      if (hintEl) hintEl.textContent = CURRENCY + ' — auto-calculated from items above';
    } else {
      // No items → allow free entry
      manualEl.readOnly = false;
      manualEl.style.background = '';
      manualEl.style.color      = '';
      if (hintEl) hintEl.textContent = CURRENCY + ' — optional, auto-filled when using items above';
    }
  }
}
...
async function saveOpFromModal(id) {
  ...
  // Collect procedure list; use first procedureId on the operation row for backward-compat
  const procs = collectOpProcedures();
  const firstProcId = procs[0]?.procedureId || null;
  ...
  const op = {
    ...
    commissionOverride: null   // commission driven by procedures now
  };

  try {
    const savedOp = await saveOperation(op);
    // Save multi-procedure list
    await saveOperationProcedures(savedOp.id, procs);
    // Save items consumed (deducts from inventory + stores material_cost)
    const items = collectOpItems();
    const manualCost = parseFloat(document.getElementById('f-manual-cost')?.value || 0);
    await saveOperationItems(savedOp.id, items, me.branch_id, manualCost);
    closeModal();
