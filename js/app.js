...
async function goto(pageId) {
  ...
  switch (pageId) {
    ...
    case 'sales':        await pgSales();        break;
    ...
    case 'deposits':     await pgDeposits();     break;
    case 'notes':        await pgDailyNotes();   break;
...
// ─── Appointments State ─────────────────────────
let _apptCalMonth = new Date().getMonth() + 1;
let _apptCalYear  = new Date().getFullYear();
let _apptCalDate  = new Date().toISOString().split('T')[0];
let _allAppts     = [];
...
async function pgAppointments() {
  ...
  document.getElementById('pageContent').innerHTML = `
    <div class="flex-center gap8 mb16" style="flex-wrap:wrap">
      <button class="btn btn-secondary btn-sm" onclick="apptCalNav(-1)">◀ Prev</button>
      <strong id="apptCalTitle" style="font-size:15px;min-width:160px;text-align:center"></strong>
      <button class="btn btn-secondary btn-sm" onclick="apptCalNav(1)">Next ▶</button>
    ...
    <!-- Calendar grid -->
    <div class="card mb16">
      <div id="apptCalGrid" style="padding:16px"></div>
    </div>

    <!-- Day appointment list -->
    <div class="card">
      <div class="card-header">
        <div class="card-title" id="apptDayTitle">Click a day to see appointments</div>
        <div class="card-sub" id="apptDayCount"></div>
      </div>
      <div id="apptDayList"><div class="empty-state" style="padding:24px"><div class="ei">📅</div><p>Select a day on the calendar above.</p></div></div>
    </div>
  `;

  await loadApptCalendar();
}

async function loadApptCalendar() {
  // Fetch entire month of appointments
  const y = _apptCalYear, m = _apptCalMonth;
  const dateFrom = `${y}-${String(m).padStart(2,'0')}-01`;
  const dateTo   = new Date(y, m, 0).toISOString().split('T')[0];
  _allAppts = await getAppointments({ dateFrom, dateTo });

  // Update calendar title
  const title = document.getElementById('apptCalTitle');
  if (title) title.textContent = new Date(y, m-1, 1).toLocaleDateString('en', { month: 'long', year: 'numeric' });

  renderCalendarGrid();
  // Refresh day list for the currently selected date
  renderApptDayList(_apptCalDate);
}

function apptCalNav(dir) {
  _apptCalMonth += dir;
  if (_apptCalMonth > 12) { _apptCalMonth = 1; _apptCalYear++; }
  if (_apptCalMonth < 1)  { _apptCalMonth = 12; _apptCalYear--; }
  loadApptCalendar();
}
...
function renderCalendarGrid() {
  ...
  const apptDays = {};
  _allAppts.forEach(a => {
    apptDays[a.date] = (apptDays[a.date] || 0) + 1;
  });

  const firstDay = new Date(y, m-1, 1);
  const daysInMonth = new Date(y, m, 0).getDate();
  let startDow = firstDay.getDay(); // 0=Sun, adapt to Mon-start
  startDow = (startDow + 6) % 7;   // Mon=0 ... Sun=6

  const dayLabels = ['Mo','Tu','We','Th','Fr','Sa','Su'];
  let html = `
    <div style="display:grid;grid-template-columns:repeat(7,1fr);gap:4px;text-align:center">
      ${dayLabels.map(d=>`<div style="font-size:11px;font-weight:700;color:var(--ink3);padding:4px 0">${d}</div>`).join('')}
  `;
  ...
  for (let day = 1; day <= daysInMonth; day++) {
    ...
    const isSelected = dateStr === _apptCalDate;
    const count      = apptDays[dateStr] || 0;

    const bg    = isSelected ? 'var(--teal)' : isToday ? 'var(--teal-light)' : 'var(--bg2)';
    const color = isSelected ? '#fff' : isToday ? 'var(--teal)' : 'var(--ink1)';
    const dot   = count > 0 ? `<div style="width:6px;height:6px;border-radius:50%;background:${isSelected?'#fff':'var(--teal)'};margin:0 auto;margin-top:2px"></div>` : '';

    html += `
      <div onclick="selectApptDay('${dateStr}')"
        style="padding:6px 4px;border-radius:8px;cursor:pointer;background:${bg};color:${color};font-size:13px;font-weight:600;user-select:none;min-height:40px">
        ${day}
        ${count > 0 ? `<div style="font-size:10px;font-weight:400;opacity:0.85">${count} appt</div>` : ''}
        ${dot}
      </div>
    `;
  }
...
function selectApptDay(dateStr) {
  _apptCalDate = dateStr;
  // Re-draw grid to update selected highlight
  renderCalendarGrid();
  renderApptDayList(dateStr);
}

function renderApptDayList(dateStr) {
  const dayTitle = document.getElementById('apptDayTitle');
  const dayCount = document.getElementById('apptDayCount');
  const dayList  = document.getElementById('apptDayList');
  if (!dayList) return;

  const dayAppts = _allAppts.filter(a => a.date === dateStr);
  const fmt = new Date(dateStr + 'T00:00:00').toLocaleDateString('en', { weekday:'long', year:'numeric', month:'long', day:'numeric' });
  if (dayTitle) dayTitle.textContent = fmt;
  if (dayCount) dayCount.textContent = `${dayAppts.length} appointment${dayAppts.length !== 1 ? 's' : ''}`;

  if (dayAppts.length === 0) {
    dayList.innerHTML = `<div class="empty-state" style="padding:24px"><div class="ei">📅</div><p>No appointments on this day.</p></div>`;
    return;
  }
  ...
  dayList.innerHTML = `
    <div class="tbl-wrap"><table>
      <thead><tr>
        <th>Time</th><th>Patient</th><th>Phone</th><th>Doctor</th><th>Service</th><th>Status</th><th>Actions</th>
      </tr></thead>
      <tbody>
        ${dayAppts.map(a => `
          <tr>
            <td><strong>${formatTime12(a.time)}</strong></td>
            <td>${esc(a.customer_name)}</td>
            <td>${esc(a.customer_phone)}</td>
            <td>${esc(a.doctor?.name||'-')}</td>
            <td>${esc(a.procedure?.name||'-')}</td>
            <td>${apptStatusBadge(a.status)}</td>
            <td>
              <div class="flex-center gap6">
                ${hasPerm('manage_appointments')
                  ? `<button class="btn btn-secondary btn-xs" onclick="openApptEditModal('${a.id}')">✏️ Edit</button>`
                  : ''}
                ${a.status !== 'cancelled' && hasPerm('manage_appointments')
                  ? `<button class="btn btn-danger btn-xs" onclick="quickApptStatus('${a.id}','cancelled')">✕</button>`
                  : ''}
              </div>
            </td>
          </tr>
...
function renderApptTable(appts) {
  renderApptDayList(_apptCalDate); // kept for backward compat
}

async function quickApptStatus(id, status) {
  try {
    await setAppointmentStatus(id, status);
    await loadApptCalendar();
    showToast('success','Updated','Appointment status changed.');
  } catch(e) { showToast('error','Error',e.message); }
}

function openApptEditModal(apptId) {
  const a = _allAppts.find(x => x.id === apptId);
  if (!a) return;
  const statuses = [
  ...
  openModal('Edit Appointment', `
    <div class="g2">
      <div class="field"><label>Patient Name *</label>
        <input type="text" id="ea-name" value="${esc(a.customer_name)}"></div>
      <div class="field"><label>Phone *</label>
        <input type="text" id="ea-phone" value="${esc(a.customer_phone)}"></div>
    </div>
    <div class="g2">
      <div class="field"><label>Doctor</label>
        <select id="ea-doctor">
          <option value="">— No doctor —</option>
          ${getDoctors().map(d=>`<option value="${d.id}" ${a.doctor_id===d.id?'selected':''}>${esc(d.name)}</option>`).join('')}
        </select></div>
      <div class="field"><label>Service</label>
        <select id="ea-proc">
          <option value="">— No service —</option>
          ${_procedures.map(p=>`<option value="${p.id}" ${a.procedure_id===p.id?'selected':''}>${esc(p.name)}</option>`).join('')}
        </select></div>
    </div>
    <div class="g2">
      ...
      <div class="field"><label>Time *</label>
        <select id="ea-time">
          ${TIME_SLOTS.map(t=>`<option value="${t}" ${a.time?.startsWith(t)?'selected':''}>${formatTime12(t)}</option>`).join('')}
        </select></div>
    </div>
    ${hasPerm('manage_appointments') || isManager() ? `
    <div class="field"><label>Status</label>
      <select id="ea-status">
        ${statuses.map(s=>`<option value="${s.value}" ${a.status===s.value?'selected':''}>${s.label}</option>`).join('')}
      </select></div>` : ''}
    <div class="field"><label>Notes</label>
      <textarea id="ea-notes" rows="2" dir="auto">${esc(a.notes||'')}</textarea></div>
  `, `saveApptEdit('${apptId}')`);
}
...
async function saveApptEdit(id) {
  ...
  if (!name || !phone) { showToast('error','Required','Name and phone are required.'); return; }
  try {
    await saveAppointment({
      id,
      customerName:  name,
      customerPhone: phone,
      doctorId:      document.getElementById('ea-doctor')?.value   || null,
      procedureId:   document.getElementById('ea-proc')?.value     || null,
      date:          document.getElementById('ea-date')?.value,
      time:          document.getElementById('ea-time')?.value,
      status:        document.getElementById('ea-status')?.value   || undefined,
      notes:         document.getElementById('ea-notes')?.value.trim() || null
    });
    closeModal();
    showToast('success','Saved','Appointment updated.');
    await loadApptCalendar();
  } catch(e) { showToast('error','Error',e.message); }
}
...
function staffForm(staff) {
  ...
  return `
    ...
    <!-- Specialist type (only for specialist role) -->
    <div id="sf-spec-type-row" style="display:${(staff?.role||'specialist')==='specialist'?'block':'none'};margin-bottom:14px">
      <div class="field">
        <label>Specialist Type</label>
        <select id="sf-spec-type" onchange="onSpecTypeChange(this.value)">
          <option value="standard"     ${(staff?.specialist_type||'standard')==='standard'?'selected':''}>Standard (flat commission per session)</option>
          <option value="nutritionist" ${staff?.specialist_type==='nutritionist'?'selected':''}>🥗 Nutritionist (% of session total)</option>
          <option value="men_laser"    ${staff?.specialist_type==='men_laser'?'selected':''}>♂ Men's Laser (% of session total)</option>
        </select>
      </div>
    </div>
    <div class="g2">
      <div class="field"><label>Monthly Salary (${CURRENCY})</label>
        <input type="number" id="sf-salary" value="${staff?.salary||0}" min="0" step="0.01">
      </div>
      <div class="field">
        <label id="sf-comm-label">Commission
          <span id="sf-comm-unit">${['nutritionist','men_laser'].includes(staff?.specialist_type) ? '(% of session)' : `(${CURRENCY}/session)`}</span>
        </label>
        <input type="number" id="sf-comm" value="${staff?.commission_rate||0}" min="0" step="0.01"
          placeholder="${['nutritionist','men_laser'].includes(staff?.specialist_type) ? 'e.g. 30 for 30%' : 'JD per session'}">
      </div>
    </div>
  `;
}
...
function onStaffRoleChange(role) {
  ...
  const specTypeRow = document.getElementById('sf-spec-type-row');
  if (specTypeRow) specTypeRow.style.display = role === 'specialist' ? 'block' : 'none';
  // Update commission label to match the current specialist type (or reset to JD if not a specialist)
  const currentSpecType = role === 'specialist' ? (document.getElementById('sf-spec-type')?.value || 'standard') : 'standard';
  onSpecTypeChange(currentSpecType);
}

// Update commission label when specialist type changes
function onSpecTypeChange(type) {
  const unitEl  = document.getElementById('sf-comm-unit');
  const commIn  = document.getElementById('sf-comm');
  if (!unitEl) return;
  const isPct = type === 'nutritionist' || type === 'men_laser';
  unitEl.textContent   = isPct ? '(% of session)' : `(${CURRENCY}/session)`;
  if (commIn) commIn.placeholder = isPct ? 'e.g. 30 for 30%' : 'JD per session';
}
...
async function saveUserFromModal(id) {
  ...
  try {
    await saveProfile({
      ...
      specialistType:  document.getElementById('sf-spec-type')?.value || 'standard',
...
let _salesData    = [];
let _salesDateFrom = new Date().toISOString().split('T')[0].slice(0,8) + '01'; // first of month
let _salesDateTo   = new Date().toISOString().split('T')[0];
let _saleItems    = [];    // items in the add/edit sale modal
let _saleInventory = [];

async function pgSales() {
  if (!hasPerm('view_operations')) { showToast('error','Access Denied','You cannot view sales.'); return; }
  setMeta('Product Sales', 'Operations › Product Sales');

  document.getElementById('pageContent').innerHTML = `
    <div class="flex-center gap8 mb16" style="flex-wrap:wrap">
      <div class="search-box">
        <span>🔍</span>
        <input type="text" id="salesSearch" placeholder="Search patient…" oninput="loadSales()">
      </div>
      <label style="font-size:12px;color:var(--ink3)">From</label>
      <input type="date" id="salesFrom" value="${_salesDateFrom}" style="padding:8px 12px;border:1.5px solid var(--border);border-radius:8px;font-family:inherit;font-size:13px" onchange="_salesDateFrom=this.value;loadSales()">
      <label style="font-size:12px;color:var(--ink3)">To</label>
      <input type="date" id="salesTo" value="${_salesDateTo}" style="padding:8px 12px;border:1.5px solid var(--border);border-radius:8px;font-family:inherit;font-size:13px" onchange="_salesDateTo=this.value;loadSales()">
      <span class="ml-auto"></span>
      ${hasPerm('edit_operations') ? `<button class="btn btn-primary btn-sm" onclick="openAddSaleModal()">+ New Sale</button>` : ''}
    </div>

    <div class="card">
      <div class="card-header">
        <div>
          <div class="card-title">🛍️ Product Sales</div>
          <div class="card-sub" id="salesCountLabel">Loading…</div>
        </div>
      </div>
      <div id="salesTableWrap">
        <div class="empty-state"><div class="spinner"></div><p>Loading sales…</p></div>
      </div>
    </div>
  `;
...
async function loadSales() {
  ...
  const dateFrom = document.getElementById('salesFrom')?.value   || _salesDateFrom;
  const dateTo   = document.getElementById('salesTo')?.value     || _salesDateTo;

  _salesData = await getProductSales({ dateFrom, dateTo, search: search || undefined });
  renderSalesTable();
}

function renderSalesTable() {
  const label = document.getElementById('salesCountLabel');
  if (label) label.textContent = `${_salesData.length} sales`;

  const wrap = document.getElementById('salesTableWrap');
  if (!wrap) return;

  if (_salesData.length === 0) {
    wrap.innerHTML = `<div class="empty-state"><div class="ei">🛍️</div><p>No sales in this period. <button class="btn btn-primary btn-sm" onclick="openAddSaleModal()">Add first sale</button></p></div>`;
    return;
  }

  const grandTotal = _salesData.reduce((s, sale) => s + (sale.total || 0), 0);

  wrap.innerHTML = `
    <div class="tbl-wrap">
      <table>
        <thead><tr>
          <th>Date</th><th>Patient</th><th>File #</th><th>Sold By</th>
          <th>Cash</th><th>Visa</th><th>CliQ</th><th>Discount</th><th>Total</th>
          <th>Notes</th>
        ...
        <tbody>
          ${_salesData.map(s => `<tr>
            ...
            <td>${esc(s.file_number||'-')}</td>
            <td class="fs12">${esc(s.seller?.name||'—')}</td>
            <td>${s.payment_cash>0?`<span class="pay-chip pay-cash">💵 ${s.payment_cash.toFixed(2)}</span>`:'—'}</td>
            <td>${s.payment_visa>0?`<span class="pay-chip pay-visa">💳 ${s.payment_visa.toFixed(2)}</span>`:'—'}</td>
            <td>${s.payment_cliq>0?`<span class="pay-chip pay-cliq">📲 ${s.payment_cliq.toFixed(2)}</span>`:'—'}</td>
            <td>${s.discount>0?`<span style="color:var(--red);font-size:12px">-${s.discount.toFixed(2)}</span>`:'—'}</td>
            <td style="font-weight:700;color:var(--teal)">${(s.total||0).toFixed(2)} ${CURRENCY}</td>
            <td class="fs12 c-ink3">${esc(s.notes||'')}</td>
            ${hasPerm('edit_operations') ? `<td>
              <div class="flex-center gap8">
                <button class="btn btn-secondary btn-xs" onclick="openViewSaleModal('${s.id}')">👁</button>
                ${hasPerm('delete_operations') ? `<button class="btn btn-danger btn-xs" onclick="confirmDeleteSale('${s.id}')">🗑</button>` : ''}
              </div>
            </td>` : ''}
          </tr>`).join('')}
        </tbody>
        <tfoot><tr>
          ...
          <td style="font-weight:800;color:var(--teal)">${grandTotal.toFixed(2)} ${CURRENCY}</td>
          <td colspan="${hasPerm('edit_operations')?2:1}"></td>
        </tr></tfoot>
      </table>
    </div>
  `;
...
function buildSaleItemRow(idx, item = {}) {
  const opts = _saleInventory.map(inv =>
    `<option value="${esc(inv.id)}" data-cost="${inv.cost||0}" ${item.inventoryId===inv.id?'selected':''}>${esc(inv.name)} (${esc(inv.unit||'pcs')})</option>`
  ).join('');
  return `
    <div id="sale-item-row-${idx}" style="display:grid;grid-template-columns:1fr 80px 90px auto;gap:8px;align-items:center;margin-bottom:8px">
      <select onchange="onSaleItemSelect(${idx},this.value)" style="padding:6px 8px;border:1.5px solid var(--border);border-radius:7px;font-size:13px">
        <option value="">— Select product —</option>
        ${opts}
      </select>
      <input type="number" id="sale-item-qty-${idx}" value="${item.qty||1}" min="0.001" step="0.001" placeholder="Qty"
      ...
      <input type="number" id="sale-item-price-${idx}" value="${item.unitPrice||0}" min="0" step="0.01" placeholder="Unit price"
        style="padding:6px 8px;border:1.5px solid var(--teal-mid);border-radius:7px;font-size:13px;text-align:right"
        oninput="calcSaleTotal()">
      <button type="button" onclick="removeSaleItem(${idx})" style="padding:4px 10px;border-radius:6px;border:1.5px solid var(--border);background:#fff;color:var(--red);cursor:pointer;font-size:13px">🗑</button>
    </div>
  `;
}
...
function buildSaleForm() {
  const today = new Date().toISOString().split('T')[0];
  const branchStaff = (_profiles||[]).filter(p => p.branch_id === me.branch_id);
  return `
    <div class="g2">
      <div class="field"><label>Patient Name *</label>
        <input type="text" id="sf-pat" placeholder="Full name" required></div>
      <div class="field"><label>File Number</label>
        <input type="text" id="sf-file" placeholder="e.g. 4061"></div>
    </div>
    <div class="g2">
      <div class="field" style="max-width:200px"><label>Date *</label>
        <input type="date" id="sf-date" value="${today}"></div>
      <div class="field"><label>Sold By 👤</label>
        <select id="sf-sold-by">
          <option value="">— Select staff member —</option>
          ${branchStaff.map(p=>`<option value="${p.id}" ${p.id===me.id?'selected':''}>${esc(p.name)}</option>`).join('')}
        </select>
      </div>
    </div>
    ...
    <div style="border:1.5px solid var(--border);border-radius:10px;padding:14px;margin-bottom:14px">
      <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:10px">
        <div style="font-weight:600;font-size:13px;color:var(--ink2)">🛒 Products Sold</div>
        <button type="button" onclick="addSaleItem()" style="padding:4px 12px;border-radius:6px;border:1.5px solid var(--teal);background:var(--teal-light);color:var(--teal);font-size:12px;font-weight:600;cursor:pointer">+ Add Product</button>
      </div>
      <div id="sf-items">
        <div id="sf-items-empty" style="color:var(--ink3);font-size:12px;padding:6px 0">Click "+ Add Product" to add items to this sale.</div>
      </div>
      <div style="margin-top:8px;text-align:right;font-size:13px;color:var(--ink3)">
        Subtotal: <strong id="sf-subtotal" style="color:var(--teal)">0.00 ${CURRENCY}</strong>
      </div>
    </div>

    <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:12px">
      <div class="field"><label>💵 Cash</label>
        <input type="number" id="sf-cash" value="0" min="0" step="0.01" class="num-input" style="width:100%" oninput="calcSaleTotal()"></div>
      <div class="field"><label>💳 Visa</label>
        <input type="number" id="sf-visa" value="0" min="0" step="0.01" class="num-input" style="width:100%" oninput="calcSaleTotal()"></div>
      <div class="field"><label>📲 CliQ</label>
        <input type="number" id="sf-cliq" value="0" min="0" step="0.01" class="num-input" style="width:100%" oninput="calcSaleTotal()"></div>
      <div class="field"><label>🏷 Discount</label>
        <input type="number" id="sf-disc" value="0" min="0" step="0.01" class="num-input" style="width:100%" oninput="calcSaleTotal()"></div>
    </div>
    <div style="display:flex;align-items:center;justify-content:flex-end;gap:8px;margin-bottom:8px">
      ...
      <span id="sf-total-disp" style="font-size:20px;font-weight:700;color:var(--teal);font-family:'Playfair Display',serif">0.00 ${CURRENCY}</span>
    </div>
    <div class="field"><label>Notes</label>
      <textarea id="sf-notes" rows="2" placeholder="Any notes…"></textarea></div>
  `;
}
...
async function openAddSaleModal() {
  ...
  _saleItems    = [];
  _saleInventory = await getInventory();
  openModal('🛍️ New Product Sale', buildSaleForm(), [
    { label: 'Cancel',    cls: 'btn-secondary', action: 'closeModal()' },
    { label: 'Save Sale', cls: 'btn-primary',   action: 'saveSaleFromModal()' }
  ], 'modal-lg');
}

function addSaleItem() {
  const emptyEl = document.getElementById('sf-items-empty');
  if (emptyEl) emptyEl.remove();
  const idx = _saleItems.length;
  _saleItems.push({ inventoryId: '', qty: 1, unitPrice: 0 });
  const container = document.getElementById('sf-items');
  if (container) container.insertAdjacentHTML('beforeend', buildSaleItemRow(idx, _saleItems[idx]));
}
...
function removeSaleItem(idx) {
  ...
  if (_saleItems.length === 0) {
    container.innerHTML = `<div id="sf-items-empty" style="color:var(--ink3);font-size:12px;padding:6px 0">Click "+ Add Product" to add items to this sale.</div>`;
  } else {
    container.innerHTML = _saleItems.map((item, i) => buildSaleItemRow(i, item)).join('');
  }
  calcSaleTotal();
}

function onSaleItemSelect(idx, invId) {
  const inv = _saleInventory.find(i => i.id === invId);
  if (inv) {
    const priceEl = document.getElementById(`sale-item-price-${idx}`);
    if (priceEl) priceEl.value = inv.cost || 0;
    _saleItems[idx].inventoryId = invId;
    _saleItems[idx].unitPrice   = inv.cost || 0;
  }
  calcSaleTotal();
}
...
function calcSaleTotal() {
  let subtotal = 0;
  _saleItems.forEach((_, idx) => {
    const qty   = parseFloat(document.getElementById(`sale-item-qty-${idx}`)?.value   || 0);
    const price = parseFloat(document.getElementById(`sale-item-price-${idx}`)?.value || 0);
    subtotal += qty * price;
  });
  const subEl = document.getElementById('sf-subtotal');
  if (subEl) subEl.textContent = `${subtotal.toFixed(2)} ${CURRENCY}`;

  const cash = parseFloat(document.getElementById('sf-cash')?.value  || 0);
  const visa = parseFloat(document.getElementById('sf-visa')?.value  || 0);
  const cliq = parseFloat(document.getElementById('sf-cliq')?.value  || 0);
  const disc = parseFloat(document.getElementById('sf-disc')?.value  || 0);
  const total = cash + visa + cliq - disc;
  const totEl = document.getElementById('sf-total-disp');
  if (totEl) totEl.textContent = `${total.toFixed(2)} ${CURRENCY}`;
}

function collectSaleItems() {
  return _saleItems.map((item, idx) => {
    const inventoryId = document.querySelector(`#sale-item-row-${idx} select`)?.value || item.inventoryId;
    const qty         = parseFloat(document.getElementById(`sale-item-qty-${idx}`)?.value   || item.qty   || 0);
    const unitPrice   = parseFloat(document.getElementById(`sale-item-price-${idx}`)?.value || item.unitPrice || 0);
    return { inventoryId, qty, unitPrice };
  }).filter(i => i.inventoryId && i.qty > 0);
}

async function saveSaleFromModal() {
  const patientName = document.getElementById('sf-pat')?.value.trim();
  if (!patientName) { showToast('error','Required','Patient name is required.'); return; }

  const sale = {
    date:        document.getElementById('sf-date').value,
    patientName,
    fileNumber:  document.getElementById('sf-file')?.value.trim() || null,
    soldBy:      document.getElementById('sf-sold-by')?.value     || null,
    cash:        parseFloat(document.getElementById('sf-cash')?.value  || 0),
    visa:        parseFloat(document.getElementById('sf-visa')?.value  || 0),
    cliq:        parseFloat(document.getElementById('sf-cliq')?.value  || 0),
    discount:    parseFloat(document.getElementById('sf-disc')?.value  || 0),
    notes:       document.getElementById('sf-notes')?.value.trim() || null
  };

  const items = collectSaleItems();
  if (items.length === 0) { showToast('error','Required','Add at least one product to the sale.'); return; }

  try {
    await saveProductSale(sale, items);
    closeModal();
    showToast('success','Sale Saved',`${items.length} product(s) sold to ${esc(patientName)}.`);
    await loadSales();
  } catch(e) { showToast('error','Save Failed', e.message); }
}

async function openViewSaleModal(id) {
  const sale = _salesData.find(s => s.id === id);
  if (!sale) return;
  ...
  const itemsHtml = items.length > 0
    ? `<table style="width:100%;border-collapse:collapse;margin-top:10px">
        <thead><tr>
          <th style="text-align:left;padding:6px;border-bottom:1px solid var(--border);font-size:12px">Product</th>
          <th style="text-align:right;padding:6px;border-bottom:1px solid var(--border);font-size:12px">Qty</th>
          <th style="text-align:right;padding:6px;border-bottom:1px solid var(--border);font-size:12px">Unit Price</th>
          <th style="text-align:right;padding:6px;border-bottom:1px solid var(--border);font-size:12px">Subtotal</th>
        </tr></thead>
        <tbody>
          ${items.map(i => `<tr>
            <td style="padding:6px;font-size:13px">${esc(i.item?.name||'Unknown')}</td>
            <td style="padding:6px;text-align:right;font-size:13px">${i.qty} ${esc(i.item?.unit||'pcs')}</td>
            <td style="padding:6px;text-align:right;font-size:13px">${i.unit_price.toFixed(2)} ${CURRENCY}</td>
            <td style="padding:6px;text-align:right;font-size:13px;font-weight:600">${(i.qty*i.unit_price).toFixed(2)} ${CURRENCY}</td>
          </tr>`).join('')}
        </tbody>
      </table>`
    : '<p style="color:var(--ink3);font-size:13px">No items recorded.</p>';

  const total = (sale.payment_cash||0)+(sale.payment_visa||0)+(sale.payment_cliq||0)-(sale.discount||0);
  openModal(`🛍️ Sale Details — ${esc(sale.patient_name)}`, `
    <div class="g2" style="margin-bottom:14px">
      <div><span class="c-ink3 fs12">Date</span><div style="font-weight:600">${sale.date}</div></div>
      <div><span class="c-ink3 fs12">File #</span><div>${esc(sale.file_number||'—')}</div></div>
    </div>
    <div style="margin-bottom:14px">
      <div style="font-weight:600;font-size:13px;margin-bottom:6px">Products Sold</div>
      ${itemsHtml}
    </div>
    <div style="display:flex;gap:16px;flex-wrap:wrap;font-size:13px;margin-bottom:8px">
      ${sale.payment_cash>0?`<span>💵 Cash: <strong>${sale.payment_cash.toFixed(2)}</strong></span>`:''}
      ${sale.payment_visa>0?`<span>💳 Visa: <strong>${sale.payment_visa.toFixed(2)}</strong></span>`:''}
      ${sale.payment_cliq>0?`<span>📲 CliQ: <strong>${sale.payment_cliq.toFixed(2)}</strong></span>`:''}
      ${sale.discount>0?`<span style="color:var(--red)">🏷 Discount: -${sale.discount.toFixed(2)}</span>`:''}
    </div>
    <div style="font-size:18px;font-weight:700;color:var(--teal)">Total: ${total.toFixed(2)} ${CURRENCY}</div>
    ${sale.notes?`<div style="margin-top:10px;color:var(--ink3);font-size:12px">${esc(sale.notes)}</div>`:''}
  `, [{ label: 'Close', cls: 'btn-secondary', action: 'closeModal()' }]);
}

async function confirmDeleteSale(id) {
  if (!hasPerm('delete_operations')) return;
  if (!confirm('Delete this sale? Inventory stock will NOT be restored. This cannot be undone.')) return;
  try {
    await deleteProductSale(id);
    showToast('success','Deleted','Sale removed.');
    await loadSales();
  } catch(e) { showToast('error','Error',e.message); }
}
...
// ─── Bank Deposits Page ──────────────────────────
let _depositsData   = [];
let _depositsFrom   = new Date(new Date().getFullYear(), new Date().getMonth(), 1).toISOString().split('T')[0];
let _depositsTo     = new Date().toISOString().split('T')[0];

async function pgDeposits() {
  if (!hasPerm('view_month_end')) { showToast('error','Access Denied',''); return; }
  setMeta('Bank Deposits', 'Finance › Bank Deposits');

  document.getElementById('pageContent').innerHTML = `
    <div class="flex-center gap8 mb16" style="flex-wrap:wrap">
      <div class="field" style="margin:0"><label style="font-size:11px">From</label>
        <input type="date" id="dep-from" value="${_depositsFrom}" onchange="_depositsFrom=this.value;loadDeposits()"></div>
      <div class="field" style="margin:0"><label style="font-size:11px">To</label>
        <input type="date" id="dep-to" value="${_depositsTo}" onchange="_depositsTo=this.value;loadDeposits()"></div>
      <span class="ml-auto"></span>
      ${isManager() ? `<button class="btn btn-primary btn-sm" onclick="openAddDepositModal()">+ Record Deposit</button>` : ''}
    </div>

    <div class="card">
      <div class="card-header">
        <div class="card-title">🏦 Bank Deposits</div>
        <div id="dep-total" class="card-sub"></div>
      </div>
...
async function loadDeposits() {
  _depositsData = await getBankDeposits({ dateFrom: _depositsFrom, dateTo: _depositsTo });
  renderDepositsTable();
}
...
function renderDepositsTable() {
  ...
  const total = _depositsData.reduce((s, d) => s + parseFloat(d.amount||0), 0);
  if (totalEl) totalEl.textContent = `Total: ${total.toFixed(2)} ${CURRENCY}`;

  if (_depositsData.length === 0) {
    wrap.innerHTML = `<div class="empty-state"><div class="ei">🏦</div><p>No deposits recorded for this period.</p></div>`;
    return;
  }

  wrap.innerHTML = `
    <div class="tbl-wrap"><table>
      <thead><tr><th>Date</th><th>Description</th><th>Amount</th><th>Recorded By</th><th>Actions</th></tr></thead>
      <tbody>
        ${_depositsData.map(d => `
          <tr>
            <td>${d.date}</td>
            <td>${esc(d.description||'—')}</td>
            <td style="font-weight:700;color:var(--teal)">${parseFloat(d.amount).toFixed(2)} ${CURRENCY}</td>
            <td>${esc(d.created_by_profile?.name||'—')}</td>
            <td>
              ${isManager() ? `
                <button class="btn btn-secondary btn-xs" onclick="openEditDepositModal('${d.id}')">✏️</button>
                <button class="btn btn-danger btn-xs" onclick="confirmDeleteDeposit('${d.id}')">🗑</button>
              ` : ''}
            </td>
          </tr>
        `).join('')}
...
function openAddDepositModal() { openEditDepositModal(null); }

function openEditDepositModal(id) {
  const dep = id ? _depositsData.find(d => d.id === id) : null;
  openModal(id ? 'Edit Deposit' : 'Record Bank Deposit', `
    <div class="g2">
      <div class="field"><label>Date *</label>
        <input type="date" id="dep-date" value="${dep?.date || new Date().toISOString().split('T')[0]}"></div>
      <div class="field"><label>Amount (${CURRENCY}) *</label>
        <input type="number" id="dep-amount" value="${dep?.amount||''}" min="0.01" step="0.01" placeholder="0.00"></div>
    </div>
    <div class="field"><label>Description (optional)</label>
      <input type="text" id="dep-desc" value="${esc(dep?.description||'')}" placeholder="e.g. Daily cash deposit"></div>
  `, `saveDepositFromModal('${id||''}')`);
}

async function saveDepositFromModal(id) {
  const amount = parseFloat(document.getElementById('dep-amount')?.value || 0);
  if (!amount || amount <= 0) { showToast('error','Required','Please enter a valid amount.'); return; }
  try {
    await saveBankDeposit({
      id:          id || undefined,
      date:        document.getElementById('dep-date').value,
      amount,
      description: document.getElementById('dep-desc')?.value.trim() || null
    });
    closeModal();
    showToast('success', id ? 'Updated' : 'Recorded', 'Deposit saved.');
    await loadDeposits();
  } catch(e) { showToast('error','Error',e.message); }
}

async function confirmDeleteDeposit(id) {
  if (!confirm('Delete this deposit record?')) return;
  try {
    await deleteBankDeposit(id);
    showToast('success','Deleted','Deposit removed.');
    await loadDeposits();
  } catch(e) { showToast('error','Error',e.message); }
}

// ─── Daily Notes Page ────────────────────────────
let _notesDate = new Date().toISOString().split('T')[0];
let _notesData = [];

async function pgDailyNotes() {
  if (!hasPerm('view_month_end')) { showToast('error','Access Denied',''); return; }
  setMeta('Daily Notes', 'Admin › Daily Notes');

  document.getElementById('pageContent').innerHTML = `
    <div class="flex-center gap8 mb16">
      <button class="btn btn-secondary btn-sm" onclick="notesNavDay(-1)">◀ Prev</button>
      <input type="date" id="notes-date" value="${_notesDate}"
        style="padding:8px 12px;border:1.5px solid var(--border);border-radius:8px;font-family:inherit;font-size:13px"
        onchange="_notesDate=this.value;loadNotes()">
      <button class="btn btn-secondary btn-sm" onclick="notesNavDay(1)">Next ▶</button>
      <span class="ml-auto"></span>
      ${isManager() ? `<button class="btn btn-primary btn-sm" onclick="openAddNoteModal()">+ Add Note</button>` : ''}
    </div>
...
function notesNavDay(dir) {
  const d = new Date(_notesDate);
  d.setDate(d.getDate() + dir);
  _notesDate = d.toISOString().split('T')[0];
  document.getElementById('notes-date').value = _notesDate;
  loadNotes();
}

async function loadNotes() {
  _notesData = await getDailyNotes(_notesDate);
  renderNotesList();
}
...
function renderNotesList() {
  ...
  if (_notesData.length === 0) {
    wrap.innerHTML = `<div class="empty-state"><div class="ei">📝</div><p>No notes for this day.${isManager()?` <button class="btn btn-primary btn-sm" onclick="openAddNoteModal()">Add one</button>`:''}</p></div>`;
    return;
  }

  wrap.innerHTML = _notesData.map(n => {
    const ts = new Date(n.created_at).toLocaleTimeString('en', { hour:'2-digit', minute:'2-digit' });
    return `
      <div class="card mb12">
        <div class="card-header" style="padding:12px 16px">
          <div>
            <span style="font-size:12px;font-weight:600;color:var(--teal)">${ts}</span>
            <span style="font-size:12px;color:var(--ink3);margin-left:8px">· ${esc(n.author?.name||'Unknown')}</span>
          </div>
          ${isManager() ? `
          <div class="flex-center gap6">
            <button class="btn btn-secondary btn-xs" onclick="openEditNoteModal('${n.id}')">✏️</button>
            <button class="btn btn-danger btn-xs" onclick="confirmDeleteNote('${n.id}')">🗑</button>
          </div>` : ''}
        </div>
        <div style="padding:0 16px 16px;font-size:14px;line-height:1.6;white-space:pre-wrap;color:var(--ink1)">${esc(n.content)}</div>
      </div>
    `;
  }).join('');
}

function openAddNoteModal() { openEditNoteModal(null); }

function openEditNoteModal(id) {
  const note = id ? _notesData.find(n => n.id === id) : null;
  openModal(id ? 'Edit Note' : 'Add Daily Note', `
    <div class="field"><label>Note</label>
      <textarea id="note-content" rows="5" dir="auto" placeholder="Write your note here…">${esc(note?.content||'')}</textarea></div>
  `, [
    { label: 'Cancel',                      cls: 'btn-secondary', action: 'closeModal()' },
    { label: id ? 'Update Note' : 'Save',   cls: 'btn-primary',   action: `saveNoteFromModal('${id||''}')` }
  ]);
}

async function saveNoteFromModal(id) {
  const content = document.getElementById('note-content')?.value.trim();
  if (!content) { showToast('error','Required','Note cannot be empty.'); return; }
  try {
    await saveDailyNote({ id: id||undefined, date: _notesDate, content });
    closeModal();
    showToast('success', id ? 'Updated' : 'Added', 'Note saved.');
    await loadNotes();
  } catch(e) { showToast('error','Error',e.message); }
}
