...
async function getOperations({ date, month, year, dateFrom, dateTo, specialistId, doctorId, branchId, search } = {}) {
  let q = supabase.from('operations_with_total').select(`
    *,
    specialist:specialist_id(id, name, specialist_type, commission_rate),
    doctor:doctor_id(id, name),
    procedure:procedure_id(id, name, name_ar, doctor_profit_pct, specialist_commission, service_type, gender, category),
    op_procedures:operation_procedures(
      id, quantity, specialist_commission_override,
      procedure:procedure_id(id, name, name_ar, specialist_commission, doctor_profit_pct)
    )
  ...
  // Flexible date range (used by reports and other pages)
