...
// Procedure categories (for specialist services)
const PROCEDURE_CATEGORIES = ['Laser', 'Skin', 'Injection', 'Consultation', 'Hair Removal', 'Other'];

// Service types for procedures
const SERVICE_TYPES = {
  doctor:     'Doctor',
...
// Gender options for procedures/services
...
// Use esc() when interpolating user data into innerHTML.
// e.g. `<td>${esc(patient.name)}</td>`
function esc(str) {
  if (str == null) return '';
  return String(str)
    .replace(/&/g, '&amp;')
...
const NAV_ITEMS = [
  ...
  { id: 'sales',       icon: '🛍️',  label: 'Product Sales',    perm: 'view_operations' },
  ...
  { id: 'monthend',    icon: '📈', label: 'Reports',           perm: 'view_month_end' },
  { id: 'deposits',    icon: '🏦', label: 'Bank Deposits',     perm: 'view_month_end' },
  { section: 'Admin' },
  { id: 'staff',       icon: '👥', label: 'Staff & Access',    perm: 'manage_staff' },
  { id: 'notes',       icon: '📝', label: 'Daily Notes',       perm: 'view_month_end' },
