...
async function loadMonthEnd() {
  ...
  // ── Doctor commissions ───────────────────────────
  // Formula matches payroll: (session_revenue - material_cost) × doctor_profit_pct / 100
  const doctorMap = {};
  ops.forEach(o => {
    if (!o.doctor_id) return;
    const gross      = (o.payment_cash||0) + (o.payment_visa||0) + (o.payment_cliq||0) + (o.payment_shot||0) - (o.discount||0);
    const netRevenue = gross - (o.material_cost || 0);
    const pct        = getOpDocPct(o);
  ...
  ops.forEach(o => {
    ...
    const commission = calcOpSpecCommission(o);
...
async function pgMyStats() {
  ...
  if (me.role === 'doctor') {
    totalEarned = ops.reduce((s,o) => {
      const gross = (o.payment_cash||0)+(o.payment_visa||0)+(o.payment_cliq||0)+(o.payment_shot||0)-(o.discount||0);
      const net   = gross - (o.material_cost||0);
      return s + net * (getOpDocPct(o) / 100);
    }, 0);
  } else if (me.role === 'specialist') {
    totalEarned = ops.reduce((s,o) => s + calcOpSpecCommission({ ...o, specialist: me }), 0) + (me.salary || 0);
  }
             ...
             <tbody>
               ${ops.map(o => {
                 ...
                 if (me.role === 'doctor') {
                   const gross = (o.payment_cash||0)+(o.payment_visa||0)+(o.payment_cliq||0)+(o.payment_shot||0)-(o.discount||0);
                   const net   = gross - (o.material_cost||0);
                   earned = net * (getOpDocPct(o) / 100);
                 } else {
                   earned = calcOpSpecCommission({ ...o, specialist: me });
                 }
                 return `<tr>
                   <td>${o.date}</td>
                   <td>${esc(o.patient_name)}</td>
                   <td>${esc(o.session_name || getOpProcNames(o) || '—')}</td>
