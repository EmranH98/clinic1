...
async function loadPayrollPeriod() {
  ...
  const specData = specialists.map(spec => {
    ...
    const sessions = specOps.map(o => {
      // Use per-operation override if set; otherwise fall back to procedure default
      const commission = o.specialist_commission_override != null
        ? o.specialist_commission_override
        : (o.procedure?.specialist_commission ?? 0);
      return { ...o, commission, hasOverride: o.specialist_commission_override != null };
    });
